...
Like ``chat_history_list.py`` but with ``ai_history_view="tree"``: conversations
are leaves in a Wunderbaum tree, folders are created via the context menu,
and dragging a conversation onto a folder moves it there. Inline rename via
click-active or F2; delete and new chat via the context menu.
...
3. Run this script: ``python examples/panels/ai/chat_history_tree.py``
...
def create_app() -> Panelini:
    ...
    return Panelini(
        title="Panelini AI Chat History Tree",
        use_ai=True,
        use_ai_history=True,
...
if __name__ == "__main__":
    serve(create_app, title="Panelini AI Chat History Tree", port=5010)
