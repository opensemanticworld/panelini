...
- `make_plot_tools(panel)` returns `[PlotByCodeTool, RunCodeTool, LoadCsvTool]` when `osw_plot_tools` cannot be imported (simulate by `monkeypatch.setitem(sys.modules, "panelini.panels.ai.plot.tools.osw_plot_tools", None)`, which produces ImportError on subsequent relative-import).
