...
`jslink` is a Panel feature that syncs properties **purely in the browser**, with no Python round-trip on every click. The `active` property is each `Tabs` widget's integer index; keeping them equal means the sidebar and main area always show the same AI.
