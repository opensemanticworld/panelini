...
- **Context menu**: `test_context_menu_visible` is marked `xfail`, since dispatching a `contextmenu` event through the shadow DOM is unreliable under Playwright, so it is expected to fail rather than flake.
