...
5. Lets the user download the beautified file, reusing the original pixels
   with the new XML embedded in the PNG `tEXt` chunk (for `.drawio.png`
   inputs) or raw XML bytes (for `.drawio` inputs).
